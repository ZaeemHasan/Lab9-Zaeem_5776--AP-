...
typedef void(*computeProduct)(struct Matrix_ *A, struct Matrix_ *B);			// Function prototype for Calculating Product
typedef void(*computeSum)(struct Matrix_ *A, struct Matrix_ *B);				// Function prototype for Calculating Sum
...
typedef void(*initializeMatrix_)(struct Matrix_ *A, int rows, int columns);		// Matrix_ class constructor
typedef void(*initializeVector_)(struct Vector_ *A, int rows);					// Vector_ class Constructor
...
typedef struct Matrix_{							//Matrix_ class
	...
	computeSum computeSm;

	computeL1Norm *vtable;
	
	print printthis; 
	initializeMatrix_ init;
}Matrix;
